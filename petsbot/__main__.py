...
def main() -> None:
    ...
    app.add_handler(MessageHandler(filters.ChatType.GROUPS & ~filters.COMMAND, handle_present_message))

    app.add_handler(CallbackQueryHandler(handle_twfix_dismiss, lambda d: json.loads(d)["type"] == "twfix.dismiss"))
    app.add_handler(CallbackQueryHandler(handle_present_callback, lambda d: json.loads(d)["type"] == "xmas.present"))
